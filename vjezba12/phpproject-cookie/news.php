<?php 
	...
	if (!isset($action)) {
		...
		if ($action == 1) {
			...
			setcookie('news_1[0]', 'US growth faster than expected');
			setcookie('news_1[1]', 'index.php?menu=' . $menu . '&action=1');
			print '
			...
			# inicijalizacija kolačića
			setcookie('news_2[0]', 'Wall Street: Tech firm surge pushes US markets higher');
			setcookie('news_2[1]', 'index.php?menu=' . $menu . '&action=2');
			print '
			...
			# inicijalizacija kolačića
			setcookie('news_3[0]', 'Hotel booking sites probed by consumer watchdog');
			setcookie('news_3[1]', 'index.php?menu=' . $menu . '&action=3');
			print '
